...
function buildCleanData(formData: any) {
  return {
    booking_id: '', // auto-generated by Payload (as you said)
...
async function sendPdfToWhatsapp(params: {
  ...
  // Use Web FormData (built-in in Next.js runtime)
...
export async function submitUmrahForm(formData: any): Promise<SubmitResponse> {
  ...
  try {
    ...
    // ====== NEW: Trigger WA sending (customer + admin) ======
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL
    if (!baseUrl) {
      // Don’t fail the whole submission if WA env is missing, but log it clearly
    ...
    if (customerPhone) {
      try {
        waCustomer = await sendPdfToWhatsapp({
          ...
          phone: customerPhone,
          bookingId,
          umrahFormData: cleanData, // send the clean data so PDF is consistent with DB
